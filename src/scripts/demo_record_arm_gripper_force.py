...
from dexterous_picking.msg import gripper_state
...
def fr_callback(frmsg, fr_file):
	fr_file.write(str(frmsg.header.stamp.secs) + ', ' + str(frmsg.header.stamp.nsecs) + ', ' + str(frmsg.vector.x) + ', ' + str(frmsg.vector.y)+ ', ' + str(frmsg.vector.z) + '\n')

def gr_callback(grmsg, gr_file):
	gr_file.write(str(grmsg.header.stamp.secs) + ', ' + str(grmsg.header.stamp.nsecs) + ', ' + str(grmsg.vals).replace(')', '').replace('(', '') + ', ' + str(grmsg.enc).replace(')', '').replace('(', '') + '\n')
...
def save_demo():
	...
	gr_pos_arr = np.zeros((1, 4))
	gr_enc_arr = np.zeros((1, 4))
	fr_time_arr = np.zeros((1, 2))
	fr_pos_arr = np.zeros((1, 3))
	...
	try:
	    ...
	    while True:
	        ...
	        if max(ctime_arr) - min(ctime_arr) < 0.01:
	            ...
	            gr_pos_arr = np.vstack((gr_pos_arr, gr_data[2:6]))
	            gr_enc_arr = np.vstack((gr_enc_arr, gr_data[6:10]))
	            
	            fr_time_arr = np.vstack((fr_time_arr, fr_data[0:2]))
	            fr_pos_arr = np.vstack((fr_pos_arr, fr_data[2:5]))
	...
	gr_enc_arr = np.delete(gr_enc_arr, 0, 0)
	...
	if len(js_time_arr) > 0:
		...
		dset_ge = fp.create_dataset('/gripper_info/gripper_encoders', data=gr_enc_arr)
...
def demo_recorder():
    try:
        ...
        sub_gr = rospy.Subscriber("/gripper/state", gripper_state, gr_callback, gr_fp)
        
        sub_fr = rospy.Subscriber("/pressure_sensor_readings", Vector3Stamped, fr_callback, fr_fp)
