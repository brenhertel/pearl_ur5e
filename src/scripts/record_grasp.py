...
def gp_callback(gpmsg, gp_file):
	gp_file.write(str(gpmsg.header.stamp.secs) + ', ' + str(gpmsg.header.stamp.nsecs) + ', ' + str(gpmsg.vector.x) + ', ' + str(gpmsg.vector.y) + ', ' + str(gpmsg.vector.z) + '\n')
...
def save_demo():
	...
	gp_time_arr = np.zeros((1, 2))
	gp_pos_arr = np.zeros((1, 3))
	...
	try:
	    ...
	    while True:
	        ...
	        gp_time = gp_data[0] + (gp_data[1] * 10.0**-9)
	        ctime_arr = [js_time, tf_time, wr_time, gr_time, fr_time]
	        #print(ctime_arr)
	        if max(ctime_arr) - min(ctime_arr) < 0.05:
	            ...
	            gp_time_arr = np.vstack((gp_time_arr, gp_data[0:2]))
	            gp_pos_arr = np.vstack((gp_pos_arr, gp_data[2:5]))
	...
	gp_time_arr = np.delete(gp_time_arr, 0, 0)
	gp_pos_arr = np.delete(gp_pos_arr, 0, 0)
	...
	if len(js_time_arr) > 0:
		...
		dset_gpt = fp.create_dataset('/object_info/object_time', data=gp_time_arr)
		dset_gpp = fp.create_dataset('/object_info/object_position', data=gp_pos_arr)
...
def end_record(js_file, tf_file, wr_file, gr_file, fr_file, gp_file):
    print(time.time())
...
def demo_recorder():
    try:
        ...
        #create grasp data file
        gp_fp = open('grasp_data.txt', 'w')
        ...
        sub_fr = rospy.Subscriber("/grasp_detection/object_location3D", Vector3Stamped, gp_callback, gp_fp)
    	
    		
        rospy.loginfo('Recording has started')
    		
        rospy.spin()
        end_record(js_fp, tf_fp, wr_fp, gr_fp, fr_fp, gp_fp)
