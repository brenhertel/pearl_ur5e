...
class MoveGroupPythonInterface(object):
  ...
  def plan_cartesian_path(self, scale=1):
    ...
    for i in range(1, n_pts):
      ...
      wpose.position.z = repro_traj[i][2] - 0.03 - (1.05**(((n_pts-i) / n_pts)**2) - 1.0)
...
def main():
  try:
    ...
    print("Press 'Enter' to go to start")
    input()
    ur5e_arm.goto_joint_state([2.0475313663482666, -1.0723768335631867, 1.2214081923114222, -0.12611277521166997, 1.5332075357437134, -2.2718990484820765])
    
    print("Press 'Enter' to execute movement")
    input()
    ur5e_arm.goto_joint_state([1.2653254270553589, -1.2661606830409546, 1.1753705183612269, 0.12481216966595454, 0.7508789300918579, -2.2958651224719446])
    rospy.sleep(0.1)
    ur5e_arm.goto_joint_state([1.307863473892212, -0.9300701183131714, 1.2864435354815882, -0.3230603498271485, 0.7933358550071716, -2.2942610422717493])
    ur5e_arm.move_group.stop()
