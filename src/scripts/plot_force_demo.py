...
def read_data(fname):
    ...
    gripper_encs = np.array(gp.get('gripper_encoders'))
    force_time = np.array(gp.get('force_time'))
    forces = np.array(gp.get('forces'))
    gripper_data = [gripper_time, gripper_pos, gripper_encs, force_time, forces]
...
def display_data(joint_data, tf_data, wrench_data, gripper_data):
    ...
    print('gripper_encoders: ' + str(np.shape(gripper_data[2])))
    print('force_time: ' + str(np.shape(gripper_data[3])))
    print('force_vals: ' + str(np.shape(gripper_data[4])))
    return
...
def plot_gripper_data(gripper_data):
	gp_fig, (ax1, ax2, ax3) = plt.subplots(3, 1)
	gp_fig.suptitle('Gripper')
	time = gripper_data[0][:, 0] + gripper_data[0][:, 1] * (10.0**-9)
	
	ax1.plot(time, gripper_data[1][:, 0], label="adductor")
	ax1.plot(time, gripper_data[1][:, 1], label="left finger")
	ax1.plot(time, gripper_data[1][:, 2], label="right finger")
	ax1.plot(time, gripper_data[1][:, 3], label="top finger")
	
	ax2.plot(time, gripper_data[2][:, 0], label="adductor")
	ax2.plot(time, gripper_data[2][:, 1], label="left finger")
	ax2.plot(time, gripper_data[2][:, 2], label="right finger")
	ax2.plot(time, gripper_data[2][:, 3], label="top finger")
	
	
	ftime = gripper_data[3][:, 0] + gripper_data[3][:, 1] * (10.0**-9)
	ax3.plot(ftime, gripper_data[4][:, 0], label="left finger")
	ax3.plot(ftime, gripper_data[4][:, 1], label="right finger")
	ax3.plot(ftime, gripper_data[4][:, 2], label="top finger")
...
def main():
    filename = '/home/pearl/catkin_ws/src/pearl_ur5e/src/scripts/h5_files/recorded_demo 2023-11-07 12:20:17.h5'#raw_input('Enter the filename of the .h5 demo: ')
    plot_data(filename)
